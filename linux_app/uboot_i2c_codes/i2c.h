 ...
 */
static inline uchar i2c_reg_read(uchar addr, uchar reg, int mode)
{
	uchar buf;
	...
	int ret = i2c_read(addr, reg, 1, &buf, 1, mode);
    if (0 != ret)
    {
        return -1;
    }
...
static inline void i2c_reg_write(uchar addr, uchar reg, uchar val, int mode)
{
#ifdef CONFIG_8xx
	/* MPC8xx needs this.  Maybe one day we can get rid of it. */
	i2c_init(CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE);
	...
	i2c_write(addr, reg, 1, &val, 1, mode);
}
